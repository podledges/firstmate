...
Pi can optionally speak completed captain-facing replies through the local `/podle-voice` command; [Pi local voice preference](docs/configuration.md#pi-local-voice-preference-statepodle-voice) owns its setup, commands, persistence, and scope.
...
| `/debugfm`         | Diagnose and safely recover Firstmate fork, remote, worktree, wrong-base branch, validation, and push-target conflicts; `/debugFM` is the captain-facing mixed-case alias when the active harness accepts it |
