...
Pi's `/podle-voice` command toggles local speech for completed captain-facing replies when the Home Manager-provided `firstmate-tts` command is available; use `/podle-voice on`, `/podle-voice off`, or `/podle-voice status` for explicit control.
