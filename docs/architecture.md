...
A crew that declared an external wait (`paused:`) or a verified captain-held transfer normally takes the bounded `FM_PAUSE_RESURFACE_SECS` recheck instead of a busy-bound wedge escalation.
...
A foreground Lavish review can also enter that bounded captain-wait cadence, in attended and away modes, when the read-only [`fm-lavish-review-wait.sh`](../bin/fm-lavish-review-wait.sh) predicate proves health on each watcher poll; its header owns the exact evidence and supported process-binding limits.
The watcher bounds each proof attempt to eight seconds and anchors the first recheck to admission, not an older status event; pane busyness or a prose mention of Lavish alone cannot admit the wait.
If an admitted review loses proof, the watcher retains reconciliation until the idle stale threshold or busy-turn bound, allowing active feedback processing without an immediate false alert.
At that boundary, authoritative `fm-crew-state` completion clears review tracking and restores normal terminal classification; otherwise the existing wedge timer and worktree-write deferral run before any declared-pause fallback, and explicit possible-wedge wakes reach the away daemon's escalation path.
A renewed healthy proof restores the wait cadence, while unresolved failure remains tracked across watcher cycles rather than falling back to a retained `paused:` declaration.
[`tests/fm-watch-triage.test.sh`](../tests/fm-watch-triage.test.sh) covers healthy admission, lost poll or session proof, pre-bound poll loss, away-daemon delivery, subsequent cycles, and feedback through completion.
