...
Eligible `session_start` first-cycle auto-arm, compaction contrast, factory-only inertness, and lock, away-mode, and no-work/no-Relay negative controls are covered by the same portable suite.
