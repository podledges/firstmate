...
The [Pi supervision protocol](supervision-protocols/pi.md) owns first-arm eligibility, startup ordering, and the compaction distinction.
...
The same suite covers ordinary same-process session replacement for `/new`, `/resume`, `/fork`, and reload, same-instance shutdown-plus-start, stale prior-generation callbacks, repeated transitions with exactly one live cycle, disappearance of the shutting-down refusal after a valid replacement activates, and terminal quit still refusing late rearm.
It also covers eligible `session_start` first-cycle auto-arm for those replacement reasons versus compaction, factory-only inertness, and lock, away-mode, and no-work/no-Relay negative controls.
