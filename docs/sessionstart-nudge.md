...
| Pi / pi-signed | Run | `.pi/extensions/fm-primary-turnend-guard.ts` maps `session_start` reasons `startup`, `new`, `resume`, `fork`, and `reload` onto wrapper sources, refines a Pi-reported `startup` to `resume` only when a continuation, resume-selection, or explicit-session flag accompanies a session header older than the current process, maps a fork flag to `fork`, handles `session_compact` as the compaction equivalent, and injects the output with `pi.sendMessage`; setup-created entries such as `--name` are not restoration evidence. | A restored session with an absent or dead lock runs `startup` instead of nudging, so acquisition finishes before watcher eligibility is checked; an owned restored session retains the silent nudge path, and another live holder remains read-only. |
...
It proves the run wrapper's source routing end to end against a real `fm-session-start.sh`, including completion-gated `--reemit` selection, generic resume delegation, owned Pi CLI continuation classification, an unrecognized source falling through to the full digest, and bounded loud delivery of an oversized Pi digest.
...
`tests/fm-pi-watch-extension.test.sh` additionally runs both Pi extensions through real startup routing and lock acquisition for absent and stale locks, including restored CLI resume/continue, and asserts one watcher without a model repair call.
