...
| `fm-handy-custom-words.sh` | Add project-name variants to Handy's `custom_words` settings array |
...
| `fm-bot-trust-filter.sh` | Recognize explicit natural-language randomized probability choices and route exact percentages to Bot Trust |
...
| `fm-agent-home-lib.sh`   | Resolve and privately prepare Firstmate-local Codex and Grok homes under the contract in [configuration.md](configuration.md#private-codex-and-grok-homes) |
...
| `fm-inbox.sh`            | The captain's out-of-band capture surface: queue a note, accept the fixed-home Hermes pilot submission, dictate one, read status, ask a side question ([hermes-inbox-pilot.md](hermes-inbox-pilot.md)) |
