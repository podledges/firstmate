...
- `MISSING: <tool> (install: <command>)` - list the missing tools to the captain with a one-line purpose each plus the printed install commands, wait for consent (one approval may cover the list), then run `bin/fm-bootstrap.sh install <approved tools...>`.
  ...
  For `no-mistakes`, this also covers an installed version older than 1.44.2, the first release whose CI monitor requires trusted no-CI evidence before treating an empty check list as ready; crewmate validation briefs also delegate gate mechanics to no-mistakes' version-matched guidance.
