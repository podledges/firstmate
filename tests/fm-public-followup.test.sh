...
test_rechain_delivers_second_post_on_same_thread() {
  ...
  out=$(FAKE_CURL_LOG="$log" FMX_NOW_OVERRIDE=1787539200 run_pf "$parent" rechain public-final-b --from public-final-a \
...
SH
  ...
  REAL_TASKS_AXI="$real_tasks" RECHAIN_FAIL_MARKER="$marker" \
    FMX_NOW_OVERRIDE=1787539200 expect_failure "rechain must expose a resumable partial add" \
  ...
  out=$(REAL_TASKS_AXI="$real_tasks" RECHAIN_FAIL_MARKER="$marker" \
    FMX_NOW_OVERRIDE=1787539200 run_pf "$home" rechain public-final-resume-b --from public-final-resume-a \
      --work-home main --work-id ship-resume --expected pr-merged) \
...
EOF
  ...
  FMX_NOW_OVERRIDE=1787539200 expect_failure "rechain must surface a failed source retirement" \
  ...
  FMX_NOW_OVERRIDE=1787539200 expect_failure "a failed retirement must not leave the source claimable by another destination" \
...
test_rechain_refuses_unclaimed_existing_destination() {
  ...
  FMX_NOW_OVERRIDE=1787539200 expect_failure "a first rechain must not adopt an unrelated existing obligation" \
